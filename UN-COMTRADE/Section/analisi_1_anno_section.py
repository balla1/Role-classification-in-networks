...
G22 = nx.read_weighted_edgelist(f'{base_path}2022_normalizzato_SECTION_XVI.txt', create_using=nx.DiGraph)
G22_c = nx.read_weighted_edgelist(f'{base_path}2022_closeness_SECTION_XVI.txt', create_using=nx.DiGraph)
G22_e = nx.read_weighted_edgelist(f'{base_path}2022_paesi_edgelist_SECTION_XVI.txt', create_using=nx.DiGraph)
...
#pagerank
pag = nx.pagerank(G22, alpha=1, weight='weight', max_iter=1000)
...
print("ordine features dalla meno rilevante alla più rilevante per laplacian scores:")
...
features_reduced = features.drop(features.columns[[4]], axis=1)
...
plt.title('in closeness')

fcm_reduced_2 = FCM(n_clusters=2, m=1.5)
features_reduced_2 = features.drop(features.columns[[4, 5]], axis=1)
...
plt.title('in closeness,  out closeness')
print("Columns eliminated: ", features.columns[[4, 5]])

fcm_reduced_3 = FCM(n_clusters=2, m=1.5)
features_reduced_3 = features.drop(features.columns[[4, 5, 9]], axis=1)
...
plt.title('in closeness,  out closeness, hubs')
plt.show()
print("Columns eliminated: ", features.columns[[4, 5, 9]])
...
# 
feat_self = features.drop(columns='in_closeness')
...
roles_percentages = pd.DataFrame(fcm.u, columns=['Role_0', 'Role_1'])
...
# 
print("laplacian score senza in closeness:",score_l)
...
print("average silhouette score con feature selection su feat_self (senza in closeness):",silhouette_score(feat_self.values, roles_percentages.idxmax(axis=1)))


print('Average silhouette score:')
print(silhouette_score(feat_self.values, roles_percentages.idxmax(axis=1)))
no_incloss_sil=silhouette_score(feat_self.values, roles_percentages.idxmax(axis=1)) #salvo il valore per confrontarlo con i successivi, 
# %%
#in ordine dalla più rilevante alla meno rilevante
print('Partition coefficient:')
print(fcm.partition_coefficient)
no_incloss_pc=fcm.partition_coefficient #salvo il valore per confrontarlo con i successivi


# without both out_closeness and selfloop

# 
feat = features.drop(columns=['in_closeness','out_closeness'])
...
# 
print("laplacian score senza in closeness e out closeness:",score_l)
...
# 
#average silhouette score con feature selection su feat (senza out_closeness e selfloop)
print("average silhouette score con feature selection su feat (senza out e in closeness):",silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))
...
no_closs_sill=silhouette_score(feat.values, roles_percentages.idxmax(axis=1)) #salvo il valore per confrontarlo con i successivi, 
# %%
#in ordine dalla più rilevante alla meno rilevante
print('Partition coefficient:')
print(fcm.partition_coefficient)
...
feat = features.drop(columns=['in_closeness','out_closeness','hubs'])
...
print("laplacian score senza senza in e out closeness e hubs:",score_l)
...
# 
#average silhouette score con feature selection su feat (senza out_closeness e selfloop)
print("average silhouette score con feature selection su feat (senza in e out closeness e hubs):",silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))
...
print('Average silhouette score:')
print(silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))
no_closs_hubs_sill=silhouette_score(feat.values, roles_percentages.idxmax(axis=1)) #salvo il valore per confrontarlo con i successivi, 

#in ordine dalla più rilevante alla meno rilevante
print('Partition coefficient:')
print(fcm.partition_coefficient)
no_closs_hubs_pc=fcm.partition_coefficient #salvo il valore per confrontarlo con i successivi

# without both out_closeness and selfloop and betweenness



feat = features.drop(columns=['in_closeness','out_closeness','out_strength','hubs'])
...
print("laplacian score senza in_closeness, out_closeness, betwenness,hubs:",score_l)
...
# 
#average silhouette score con feature selection su feat (senza out_closeness e selfloop)
print("average silhouette score con feature selection su feat (senza out_strength, out_closeness, betweenness,hubs):",silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))


print('Average silhouette score:')
print(silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))
no_closs_hubs_outstr_sill=silhouette_score(feat.values, roles_percentages.idxmax(axis=1)) #salvo il valore per confrontarlo con i successivi, 

#in ordine dalla più rilevante alla meno rilevante
print('Partition coefficient:')
print(fcm.partition_coefficient)
no_closs_hubs_outstr_pc=fcm.partition_coefficient #salvo il valore per confrontarlo con i successivi

# Il risultato è ottimo per quanto riguarda l'average silhouette score, dunque è sensato rimuovere tutte e 3 dal feature set

feat = features.drop(columns=['in_closeness','out_closeness','out_strength','hubs','clustering coefficient'])
...
print('Average silhouette score:')
print(silhouette_score(feat.values, roles_percentages.idxmax(axis=1)))
no_closs_hubs_outstr_clust_sill=silhouette_score(feat.values, roles_percentages.idxmax(axis=1)) #salvo il valore per confrontarlo con i successivi, 

#in ordine dalla più rilevante alla meno rilevante
print('Partition coefficient:')
print(fcm.partition_coefficient)
no_closs_hubs_outstr_clust_pc=fcm.partition_coefficient #salvo il valore per confrontarlo con i successivi


# without both out_closeness and selfloop and betweenness

# perchè da qui in giù non funziona? a: 
sil= [all_sil, no_closs_hubs_outstr_sill, no_closs_hubs_outstr_clust_sill]
pc=[all_pc, no_closs_hubs_outstr_pc, no_closs_hubs_outstr_clust_pc]
sil_names=['All features', 'no closeness, hubs, out strength', 'no closeness, hubs, out strength, clustering coefficient']
pc_names=['All features', 'no closeness, hubs, out strength', 'no closeness, hubs, out strength, clustering coefficient']
# Creazione dell'istogramma
